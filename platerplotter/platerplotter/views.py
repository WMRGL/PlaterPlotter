...
from problemsamples.views import pad_zeros, confirm_sample_positions
from .models import (Gel1008Csv, Plate,
					 HoldingRack, Sample, HoldingRackWell)
from problemsamples.models import RackScanner, RackScannerSample
from .config.load_config import LoadConfig
from .forms import (HoldingRackForm, PlatingForm,
					Gel1008Form, PlateSelectForm)
from problemsamples.forms import SampleSelectForm, LogIssueForm, ResolveIssueForm
...
def check_tissue_type(tissue_type):
	error = None
	accepted_values = ["Normal or Germline sample", "Liquid tumour sample", "Solid tumour sample",
					   "Abnormal tissue sample", "Omics sample"]
...
def check_sample_delivery_mode(sample_delivery_mode):
	...
	if sample_delivery_mode.title() not in accepted_values:
		error = 'Incorrect sample_delivery_mode. Received {} which is not in the list of accepted values'.format(
